...
export default function CategorySalesReportPage() {
  ...
  function exportExcel() {
    if (!categorySales.length) { alert("No data to export"); return; }
    const headers = ["Category", "Amount (AED)", "Quantity"];
    const rows = categorySales.map((r) => [r.categoryName || r.categoryId, fmt(r.totalAmount), r.totalQuantity]);
    ...
    <div className="min-h-screen bg-black p-6">
      <header className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-sky-400">Category Sales Report</h1>
      ...
      {loading ? <p className="text-slate-500">Loading...</p> : data ? (
        <div className="rounded-xl border border-slate-700 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {categorySales.map((r) => (
                <tr key={r.categoryId} className="border-b border-slate-700/50">
                  <td className="p-4 text-slate-300">{r.categoryName || r.categoryId}</td>
