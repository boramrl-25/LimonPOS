...
export default function UserShiftsReportPage() {
  ...
  useEffect(() => {
    ...
    getUserShiftEvents(dateFrom, dateTo)
      ...
      .catch((e: unknown) => {
        if (!cancelled) {
          setEvents([]);
          setError((e as Error).message || "Failed to load user shift events");
        }
      })
      .finally(() => {
    ...
    <div className="min-h-screen bg-black p-6">
      <header className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-sky-400">User Sign-in / Sign-out</h1>
            <p className="text-slate-400 text-sm">Per-user shift activity by date range</p>
          </div>
        </div>
        <ReportDateFilter
          dateFrom={dateFrom}
      ...
      {loading ? (
        ...
        <p className="text-slate-500">No user sign-in/sign-out activity for this range.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-800/80 border-b border-slate-700">
                ...
                <th className="text-left p-3 font-medium text-slate-200">User</th>
              ...
              {events.map((ev, idx) => (
                <tr key={idx} className="border-b border-slate-700/40 hover:bg-slate-800/40">
                  ...
                  <td className="p-3">
                    ...
                    >
                      {ev.action === "user_sign_in" ? "Sign-in" : "Sign-out"}
                    </span>
                  </td>
